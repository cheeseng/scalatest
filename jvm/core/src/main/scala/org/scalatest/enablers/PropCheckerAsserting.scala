    ...
    private def checkForAll[A, B, C](names: List[String], config: Parameter,
      ...
      def loop(succeededCount: Int, discardedCount: Int, aEdges: List[A], bEdges: List[B], cEdges: List[C], rnd: Randomizer, initialSizes: List[PosZInt], initSeed: Long): PropertyCheckResult = {
        ...
        result match {
          ...
          case Failure(ex) =>
            @tailrec
            def shrinkLoop(roseTreesRemaining: List[RoseTree[(A, B, C)]], mostRecentlyFailedRoseTree: RoseTree[(A, B, C)], mostRecentFailureException: Throwable, mostRecentSiblings: List[RoseTree[(A, B, C)]], shrinkLoopRnd: Randomizer, count: Int): PropertyCheckResult = {
              // println()
              // println()
              println("---------------------------------------")
              println(s"shrinkLoop $count: $roseTreesRemaining\n    $mostRecentlyFailedRoseTree\n   $mostRecentSiblings\n")
              roseTreesRemaining match {
                case Nil =>
                  // println("shrinkLoop: case Nil")
                  val (bestA, bestB, bestC) = mostRecentlyFailedRoseTree.value
                  println(s"############ BEST A: $bestA")
                  println(s"############ BEST B: $bestB")
                  println(s"############ BEST C: $bestC")
                  val shrunkArgsPassed = 
                    List(
                      if (names.isDefinedAt(0)) PropertyArgument(Some(names(0)), bestA) else PropertyArgument(None, bestA), 
                      if (names.isDefinedAt(1)) PropertyArgument(Some(names(1)), bestB) else PropertyArgument(None, bestB), 
                      if (names.isDefinedAt(2)) PropertyArgument(Some(names(2)), bestC) else PropertyArgument(None, bestC)
                    )
                  println(s"############ SHRUNK ARGS PASSED: $shrunkArgsPassed")
                  val theRes = new PropertyCheckResult.Failure(succeededCount, Some(mostRecentFailureException), names, shrunkArgsPassed, initSeed)
                  println(s"############ THE RES: $theRes")
            ...
            val roseTreeOfABC = 
              roseTreeOfA.flatMap { a => 
                new RoseTree[(A, B, C)] {
                  val value = (roseTreeOfA.value, roseTreeOfB.value, roseTreeOfC.value)
                  def shrinks(rndPassedToShrinks: Randomizer): (List[RoseTree[(A, B, C)]], Randomizer) = {
                    val (shrunkRtsOfA, nextRnd) = roseTreeOfA.shrinks(rndPassedToShrinks)
                    val (shrunkRtsOfB, nextRnd2) = roseTreeOfB.shrinks(nextRnd)
                    val (shrunkRtsOfC, nextRnd3) = roseTreeOfC.shrinks(nextRnd2)
                    (
                      // Cartesian product!
                      for {
                        rtOfA <- shrunkRtsOfA
                        rtOfB <- shrunkRtsOfB
                        rtOfC <- shrunkRtsOfC
                      } yield rtOfA.map(a => (a, rtOfB.value, rtOfC.value)), 
                      nextRnd3
                    )
                  }
                }
            ...
            println(s"JUST FAILED WITH ($roseTreeOfA, $roseTreeOfB, $roseTreeOfC)")
            val (firstLevelRoseTrees, rnd4) = roseTreeOfABC.shrinks(rnd3)
            println(s"ABOUT TO SHRINKLOOP WITH $firstLevelRoseTrees")
            shrinkLoop(firstLevelRoseTrees, roseTreeOfABC, ex, Nil, rnd4, 0)
        }
      }
    ...
    private def checkForAll[A, B, C, D](names: List[String], config: Parameter,
      ...
      def loop(succeededCount: Int, discardedCount: Int, aEdges: List[A], bEdges: List[B], cEdges: List[C], dEdges: List[D], rnd: Randomizer, initialSizes: List[PosZInt], initSeed: Long): PropertyCheckResult = {
        ...
        result match {
          ...
          case Failure(ex) =>
            @tailrec
            def shrinkLoop(roseTreesRemaining: List[RoseTree[(A, B, C, D)]], mostRecentlyFailedRoseTree: RoseTree[(A, B, C, D)], mostRecentFailureException: Throwable, mostRecentSiblings: List[RoseTree[(A, B, C, D)]], shrinkLoopRnd: Randomizer, count: Int): PropertyCheckResult = {
              // println()
              // println()
              println("---------------------------------------")
              println(s"shrinkLoop $count: $roseTreesRemaining\n    $mostRecentlyFailedRoseTree\n   $mostRecentSiblings\n")
              roseTreesRemaining match {
                case Nil =>
                  // println("shrinkLoop: case Nil")
                  val (bestA, bestB, bestC, bestD) = mostRecentlyFailedRoseTree.value
                  println(s"############ BEST A: $bestA")
                  println(s"############ BEST B: $bestB")
                  println(s"############ BEST C: $bestC")
                  println(s"############ BEST D: $bestD")
                  val shrunkArgsPassed = 
                    List(
                      if (names.isDefinedAt(0)) PropertyArgument(Some(names(0)), bestA) else PropertyArgument(None, bestA), 
                      if (names.isDefinedAt(1)) PropertyArgument(Some(names(1)), bestB) else PropertyArgument(None, bestB), 
                      if (names.isDefinedAt(2)) PropertyArgument(Some(names(2)), bestC) else PropertyArgument(None, bestC), 
                      if (names.isDefinedAt(3)) PropertyArgument(Some(names(3)), bestD) else PropertyArgument(None, bestD)
                    )
                  println(s"############ SHRUNK ARGS PASSED: $shrunkArgsPassed")
                  val theRes = new PropertyCheckResult.Failure(succeededCount, Some(mostRecentFailureException), names, shrunkArgsPassed, initSeed)
                  println(s"############ THE RES: $theRes")
                ...
                case roseTreeHead :: roseTreeTail =>
                  val (a, b, c, d) = roseTreeHead.value
                  val result: Try[T] = Try { fun(a, b, c, d) }
                  result match {
                    case Success(_) =>
                      // println("shrinkLoop: case roseTreeHead :: roseTreeTail SUCCESS!")
            ...
            val roseTreeOfABCD = 
              roseTreeOfA.flatMap { a => 
                new RoseTree[(A, B, C, D)] {
                  val value = (roseTreeOfA.value, roseTreeOfB.value, roseTreeOfC.value, roseTreeOfD.value)
                  def shrinks(rndPassedToShrinks: Randomizer): (List[RoseTree[(A, B, C, D)]], Randomizer) = {
                    val (shrunkRtsOfA, nextRnd) = roseTreeOfA.shrinks(rndPassedToShrinks)
                    val (shrunkRtsOfB, nextRnd2) = roseTreeOfB.shrinks(nextRnd)
                    val (shrunkRtsOfC, nextRnd3) = roseTreeOfC.shrinks(nextRnd2)
                    val (shrunkRtsOfD, nextRnd4) = roseTreeOfD.shrinks(nextRnd3)
                    (
                      // Cartesian product!
                      for {
                        rtOfA <- shrunkRtsOfA
                        ...
                        rtOfC <- shrunkRtsOfC
                        rtOfD <- shrunkRtsOfD
                      } yield rtOfA.map(a => (a, rtOfB.value, rtOfC.value, rtOfD.value)), 
                      nextRnd4
                    )
                  }
                }
            ...
            println(s"JUST FAILED WITH ($roseTreeOfA, $roseTreeOfB, $roseTreeOfC, $roseTreeOfD)")
            val (firstLevelRoseTrees, rnd4) = roseTreeOfABCD.shrinks(rnd3)
            println(s"ABOUT TO SHRINKLOOP WITH $firstLevelRoseTrees")
            shrinkLoop(firstLevelRoseTrees, roseTreeOfABCD, ex, Nil, rnd4, 0)
        }
      }
    ...
    private def checkForAll[A, B, C, D, E](names: List[String], config: Parameter,
      ...
      def loop(succeededCount: Int, discardedCount: Int, aEdges: List[A], bEdges: List[B], cEdges: List[C], dEdges: List[D], eEdges: List[E], rnd: Randomizer, initialSizes: List[PosZInt], initSeed: Long): PropertyCheckResult = {
        ...
        result match {
          ...
          case Failure(ex) =>
            @tailrec
            def shrinkLoop(roseTreesRemaining: List[RoseTree[(A, B, C, D, E)]], mostRecentlyFailedRoseTree: RoseTree[(A, B, C, D, E)], mostRecentFailureException: Throwable, mostRecentSiblings: List[RoseTree[(A, B, C, D, E)]], shrinkLoopRnd: Randomizer, count: Int): PropertyCheckResult = {
              // println()
              // println()
              println("---------------------------------------")
              println(s"shrinkLoop $count: $roseTreesRemaining\n    $mostRecentlyFailedRoseTree\n   $mostRecentSiblings\n")
              roseTreesRemaining match {
                case Nil =>
                  // println("shrinkLoop: case Nil")
                  val (bestA, bestB, bestC, bestD, bestE) = mostRecentlyFailedRoseTree.value
                  println(s"############ BEST A: $bestA")
                  println(s"############ BEST B: $bestB")
                  println(s"############ BEST C: $bestC")
                  println(s"############ BEST D: $bestD")
                  println(s"############ BEST E: $bestE")
                  val shrunkArgsPassed = 
                    List(
                      if (names.isDefinedAt(0)) PropertyArgument(Some(names(0)), bestA) else PropertyArgument(None, bestA), 
                      if (names.isDefinedAt(1)) PropertyArgument(Some(names(1)), bestB) else PropertyArgument(None, bestB), 
                      if (names.isDefinedAt(2)) PropertyArgument(Some(names(2)), bestC) else PropertyArgument(None, bestC), 
                      if (names.isDefinedAt(3)) PropertyArgument(Some(names(3)), bestD) else PropertyArgument(None, bestD), 
                      if (names.isDefinedAt(4)) PropertyArgument(Some(names(4)), bestE) else PropertyArgument(None, bestE)
                    )
                  println(s"############ SHRUNK ARGS PASSED: $shrunkArgsPassed")
                  val theRes = new PropertyCheckResult.Failure(succeededCount, Some(mostRecentFailureException), names, shrunkArgsPassed, initSeed)
                  println(s"############ THE RES: $theRes")
                ...
                case roseTreeHead :: roseTreeTail =>
                  val (a, b, c, d, e) = roseTreeHead.value
                  val result: Try[T] = Try { fun(a, b, c, d, e) }
                  result match {
                    case Success(_) =>
                      // println("shrinkLoop: case roseTreeHead :: roseTreeTail SUCCESS!")
            ...
            val roseTreeOfABCDE = 
              roseTreeOfA.flatMap { a => 
                new RoseTree[(A, B, C, D, E)] {
                  val value = (roseTreeOfA.value, roseTreeOfB.value, roseTreeOfC.value, roseTreeOfD.value, roseTreeOfE.value)
                  def shrinks(rndPassedToShrinks: Randomizer): (List[RoseTree[(A, B, C, D, E)]], Randomizer) = {
                    val (shrunkRtsOfA, nextRnd) = roseTreeOfA.shrinks(rndPassedToShrinks)
                    val (shrunkRtsOfB, nextRnd2) = roseTreeOfB.shrinks(nextRnd)
                    val (shrunkRtsOfC, nextRnd3) = roseTreeOfC.shrinks(nextRnd2)
                    val (shrunkRtsOfD, nextRnd4) = roseTreeOfD.shrinks(nextRnd3)
                    val (shrunkRtsOfE, nextRnd5) = roseTreeOfE.shrinks(nextRnd4)
                    (
                      // Cartesian product!
                      for {
                        rtOfA <- shrunkRtsOfA
                        ...
                        rtOfD <- shrunkRtsOfD
                        rtOfE <- shrunkRtsOfE
                      } yield rtOfA.map(a => (a, rtOfB.value, rtOfC.value, rtOfD.value, rtOfE.value)), 
                      nextRnd5
                    )
                  }
                }
              }
            println(s"JUST FAILED WITH ($roseTreeOfA, $roseTreeOfB, $roseTreeOfC, $roseTreeOfD, $roseTreeOfE)")
            val (firstLevelRoseTrees, rnd4) = roseTreeOfABCDE.shrinks(rnd3)
            println(s"ABOUT TO SHRINKLOOP WITH $firstLevelRoseTrees")
            shrinkLoop(firstLevelRoseTrees, roseTreeOfABCDE, ex, Nil, rnd4, 0)
        }
      }
    ...
    private def checkForAll[A, B, C, D, E, F](names: List[String], config: Parameter,
      ...
      def loop(succeededCount: Int, discardedCount: Int, aEdges: List[A], bEdges: List[B], cEdges: List[C], dEdges: List[D], eEdges: List[E], fEdges: List[F], rnd: Randomizer, initialSizes: List[PosZInt], initSeed: Long): PropertyCheckResult = {
        ...
        result match {
          ...
          case Failure(ex) =>
            @tailrec
            def shrinkLoop(roseTreesRemaining: List[RoseTree[(A, B, C, D, E, F)]], mostRecentlyFailedRoseTree: RoseTree[(A, B, C, D, E, F)], mostRecentFailureException: Throwable, mostRecentSiblings: List[RoseTree[(A, B, C, D, E, F)]], shrinkLoopRnd: Randomizer, count: Int): PropertyCheckResult = {
              // println()
              // println()
              println("---------------------------------------")
              println(s"shrinkLoop $count: $roseTreesRemaining\n    $mostRecentlyFailedRoseTree\n   $mostRecentSiblings\n")
              roseTreesRemaining match {
                case Nil =>
                  // println("shrinkLoop: case Nil")
                  val (bestA, bestB, bestC, bestD, bestE, bestF) = mostRecentlyFailedRoseTree.value
                  println(s"############ BEST A: $bestA")
                  println(s"############ BEST B: $bestB")
                  println(s"############ BEST C: $bestC")
                  println(s"############ BEST D: $bestD")
                  println(s"############ BEST E: $bestE")
                  println(s"############ BEST F: $bestF")
                  val shrunkArgsPassed = 
                    List(
                      if (names.isDefinedAt(0)) PropertyArgument(Some(names(0)), bestA) else PropertyArgument(None, bestA), 
                      if (names.isDefinedAt(1)) PropertyArgument(Some(names(1)), bestB) else PropertyArgument(None, bestB), 
                      ...
                      if (names.isDefinedAt(3)) PropertyArgument(Some(names(3)), bestD) else PropertyArgument(None, bestD), 
                      if (names.isDefinedAt(4)) PropertyArgument(Some(names(4)), bestE) else PropertyArgument(None, bestE), 
                      if (names.isDefinedAt(5)) PropertyArgument(Some(names(5)), bestF) else PropertyArgument(None, bestF)
                    )
                  println(s"############ SHRUNK ARGS PASSED: $shrunkArgsPassed")
                  val theRes = new PropertyCheckResult.Failure(succeededCount, Some(mostRecentFailureException), names, shrunkArgsPassed, initSeed)
                  println(s"############ THE RES: $theRes")
                ...
                case roseTreeHead :: roseTreeTail =>
                  val (a, b, c, d, e, f) = roseTreeHead.value
                  val result: Try[T] = Try { fun(a, b, c, d, e, f) }
                  result match {
                    case Success(_) =>
                      // println("shrinkLoop: case roseTreeHead :: roseTreeTail SUCCESS!")
            ...
            val roseTreeOfABCDEF = 
              roseTreeOfA.flatMap { a => 
                new RoseTree[(A, B, C, D, E, F)] {
                  val value = (roseTreeOfA.value, roseTreeOfB.value, roseTreeOfC.value, roseTreeOfD.value, roseTreeOfE.value, roseTreeOfF.value)
                  def shrinks(rndPassedToShrinks: Randomizer): (List[RoseTree[(A, B, C, D, E, F)]], Randomizer) = {
                    val (shrunkRtsOfA, nextRnd) = roseTreeOfA.shrinks(rndPassedToShrinks)
                    val (shrunkRtsOfB, nextRnd2) = roseTreeOfB.shrinks(nextRnd)
                    val (shrunkRtsOfC, nextRnd3) = roseTreeOfC.shrinks(nextRnd2)
                    val (shrunkRtsOfD, nextRnd4) = roseTreeOfD.shrinks(nextRnd3)
                    val (shrunkRtsOfE, nextRnd5) = roseTreeOfE.shrinks(nextRnd4)
                    val (shrunkRtsOfF, nextRnd6) = roseTreeOfF.shrinks(nextRnd5)
                    (
                      // Cartesian product!
                      for {
                        rtOfA <- shrunkRtsOfA
                        ...
                        rtOfE <- shrunkRtsOfE
                        rtOfF <- shrunkRtsOfF
                      } yield rtOfA.map(a => (a, rtOfB.value, rtOfC.value, rtOfD.value, rtOfE.value, rtOfF.value)), 
                      nextRnd6
                    )
                  }
                }
              }
            println(s"JUST FAILED WITH ($roseTreeOfA, $roseTreeOfB, $roseTreeOfC, $roseTreeOfD, $roseTreeOfE, $roseTreeOfF)")
            val (firstLevelRoseTrees, rnd4) = roseTreeOfABCDEF.shrinks(rnd3)
            println(s"ABOUT TO SHRINKLOOP WITH $firstLevelRoseTrees")
            shrinkLoop(firstLevelRoseTrees, roseTreeOfABCDEF, ex, Nil, rnd4, 0)
        }
      }
