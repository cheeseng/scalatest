...
object Collecting {
   ...
   * @tparam ITER any subtype of <code>Iterable</code>
   * @return <code>Collecting[E, TRAV[E]]</code> that supports <code>Iterable</code> in <code>loneElement</code> syntax
   */
  implicit def collectingNatureOfIterable[E, ITER[e] <: scala.collection.Iterable[e]]: Collecting[E, ITER[E]] = 
    new Collecting[E, ITER[E]] {
      def loneElementOf(trav: ITER[E]): Option[E] = {
        if (trav.size == 1) Some(trav.head) else None
      }
      def sizeOf(trav: ITER[E]): Int = trav.size
      def iterableFrom(collection: ITER[E]): Iterable[E] = collection
    }
