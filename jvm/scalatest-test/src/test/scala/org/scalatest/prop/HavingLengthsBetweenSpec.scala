  ...
  describe("A HavingLengthsBetween Generator for Lists") {
    describe("where from is 0") {
      ...
      it("should return an Iterator of the canonicals excluding the given values to shrink when asked to shrink a List of size 1") {
        ...
        val nonCanonicalLstGen = lists[Int].havingLength(1).filter(l => !canonicalLists.contains(l))
        val (rtNonCanonical, _, _) = nonCanonicalLstGen.next(SizeParam(1, 1, 1), List.empty, Randomizer.default)
        rtNonCanonical.shrinks(Randomizer.default)._1.map(_.value) should contain theSameElementsAs expectedLists

        val canonicalLstGen = lists[Int].havingLength(1).filter(l => canonicalLists.contains(l))
        val (rtCanonical, _, _) = canonicalLstGen.next(SizeParam(1, 1, 1), canonicalLists, Randomizer.default)
        // Ensure canonical value does not show up twice in the output
