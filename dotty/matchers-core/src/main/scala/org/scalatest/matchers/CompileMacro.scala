...
object CompileMacro {
  ...
  def assertNotCompileImpl[T](self: Expr[T], typeChecked: Expr[List[Error]], compileWord: Expr[CompileWord], pos: Expr[source.Position])(shouldOrMust: String)(using Quotes): Expr[Assertion] = {
    import quotes.reflect._
    ...
    def checkNotCompile(code: String): Expr[Assertion] = {
      ...
      errors match {
        case Error(_, _, _, _) :: _ => '{ Succeeded }
        case Nil => '{
