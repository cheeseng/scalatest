...
object ScalatestBuild extends BuildCommons with DottyBuild with NativeBuild with JsBuild {
  ...
  lazy val scalactic = project.in(file("jvm/scalactic"))
    ...
    .settings(
      ...
      Compile / packageBin / mappings ++= mappings.in(scalacticMacro, Compile, packageBin).value,
      // include the macro sources in the main source jar
      Compile / packageSrc / mappings ++= mappings.in(scalacticMacro, Compile, packageSrc).value,
      Compile / doc / sources :=
        genDocSources((Compile / sources).value ++ (sources in scalacticMacro in Compile).value,
  ...
  lazy val scalatestApp = project.in(file("scalatest-app"))
    ...
    .settings(
      ...
      Compile / packageBin / mappings ++= mappings.in(scalactic, Compile, packageBin).value,
      // include the scalactic sources in the source jar
      Compile / packageSrc / mappings ++= mappings.in(scalactic, Compile, packageSrc).value,
      // include the scalatest classes and resources in the jar
      Compile / packageBin / mappings ++= mappings.in(scalatest, Compile, packageBin).value,
      // include the scalatest sources in the source jar
      Compile / packageSrc / mappings ++= mappings.in(scalatest, Compile, packageSrc).value,
      Compile / sourceGenerators += {
